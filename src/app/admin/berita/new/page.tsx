...
export default function NewBerita() {
  ...
  const handleSave = async () => {
    if (!judul || !isi) {
      setError('Judul and isi are required')
      return
    }
    ...
    try {
      const response = await fetch('/api/berita', {
        ...
        body: JSON.stringify({ judul, isi, gambar }),
      })

      const data = await response.json()

      if (!response.ok) {
        throw new Error(data.error || 'Failed to create berita')
      }

      router.push('/admin/dashboard?tab=berita')
    } catch (err: any) {
      setError(err.message || 'Failed to create berita')
      setSaving(false)
    }
  }
  ...
  return (
    <div className="min-h-screen bg-background">
        ...
        <div className="px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            <div className="flex items-center gap-3">
              <Link href="/admin/dashboard" className="text-foreground/60 hover:text-foreground transition-colors">
                <ArrowLeft size={24} />
              </Link>
      ...
      <main className="max-w-4xl mx-auto p-4 md:p-8">
        <div className="mb-8">
          <h2 className="text-3xl font-bold mb-2">Create New Berita</h2>
          <p className="text-foreground/60">Fill in the form below to create a new berita</p>
        </div>
        ...
        <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-xl p-6 md:p-8 space-y-6">
          ...
          {/* Isi */}
          <div>
            <label className="block text-sm font-medium mb-2">Isi *</label>
            <textarea
              value={isi}
              onChange={(e) => setIsi(e.target.value)}
              rows={10}
              className="w-full px-4 py-3 border border-border rounded-xl focus:ring-2 focus:ring-primary focus:border-transparent transition-all outline-none resize-y"
              placeholder="Enter berita content"
              required
            />
          </div>
          ...
          <div className="flex items-center gap-4 pt-4">
            ...
            >
              {saving ? (
                ...
                <>
                  <Save size={20} />
                  <span>Save Berita</span>
                </>
              )}
            </button>
            <Link
              href="/admin/dashboard?tab=berita"
